...
| D-065 | Origen del `GOOGLE_REDIRECT_URI` en desarrollo | El de la interfaz (`http://localhost:4321`), no el de la API | El servidor de desarrollo hace proxy de `/api`, así que el callback llega igual, y la redirección final aterriza en la interfaz. Apuntando al puerto 8000 el usuario acababa en el origen de la API, donde `/integrations` no existe |
